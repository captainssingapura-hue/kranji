...
public final class ReadingConformance {
    ...
    public static final List<Allowance> ALLOWANCES = List.of(
            new Allowance(
                    ...
                    "The export writes a blob URL onto an anchor so the record can be "
                  + "saved as a file. The rule exists to route links through HrefManager "
                  + "so they respect the app's routing, and this is not a link: it is a "
                  + "handle to bytes the page is already holding, valid for one click and "
                  + "revoked afterwards. Reaching HrefManager would mean importing an "
                  + "unrelated app's AppLink purely to obtain the injection - a dependency "
                  + "that would misdescribe the module. The anchor itself is built through "
                  + "the branch, so the href assignment is the only exceptional line."));
