 ...
 * to withdraw as it was to claim — but the place to do either is the reader,
...
public final class KnownZiWidget
    ...
    protected List<String> constructBodyJs() {
        return List.of(
                ...
                "",
                "    // The list does not follow the record any more, so this is how it",
                "    // catches up. A claim made in the reader while this is open changes",
                "    // one row and used to rebuild the whole grid; now it changes nothing",
                "    // on screen until asked.",
                "    //",
                "    // Deliberately a plain button rather than a quiet auto-refresh: a",
                "    // list that silently reorders under a reader who is reading it is",
                "    // worse than one that waits to be told. The label says when there is",
                "    // something to catch up on.",
                "    var reload = branch.createElement('reload', 'button');",
                "    css.setClass(reload, kr_btn);",
                "    reload.type = 'button';",
                ...
                "    var __known = [];",
                "",
                "    // True once the device has answered and the list has drawn what it",
                "    // holds. Claims keep arriving after that - other panes still make",
                "    // them - and are recorded without redrawing.",
                "    var __settled = false;",
                "    var __behind = false;",
                "",
                "    function settle() {",
                ...
                "    }",
                "",
                "    // Says there is something to catch up on, rather than catching up.",
                "    function stale() {",
                "        if (__behind) return;",
                "        __behind = true;",
                "        reload.textContent = 'Reload \\u00b7 the record has moved';",
                "    }",
                "",
                "    function catchUp() {",
                "        __behind = false;",
                ...
                "                ? 'Mark a reading known while reading and it will appear here.'",
                "                : 'Nothing matches that filter.';",
                "            return;",
                "        }",
                "        status.textContent = rows.length === readings",
                "            ? 'A row is a reading. Claims are made while reading; press Reload to catch up.'",
                ...
                "    });",
                "",
                "    reload.addEventListener('click', catchUp);",
                "",
                ...
                "            reactors: {",
                "                // Recorded always, drawn only until the device answers.",
                "                //",
                "                // Every claim used to rebuild this grid, which gains or",
                "                // loses exactly one row - and the adapter's update and",
                "                // deleteRows are both no-ops, so a rebuild was the only",
                "                // way it knew how to change.",
                "                //",
                "                // Not 'take the first message': the party replies to a new",
                "                // member immediately with whatever it holds, which at boot",
                "                // is an empty set while the disk is still being read, so",
                ...
                "                    else stale();",
                "                },",
                "                // Rewritten from outside the bus. The message says that",
                "                // it happened, not what it is now, so read the device.",
                "                KnownRecordRewritten: function () {",
                "                    createKnownStore().load(null).then(function (row) {",
                "                        __known = (row && row.known) || [];",
                "                        if (__settled) stale(); else render();",
                "                    }, function () {});",
                "                }",
                "            }",
                "        });",
