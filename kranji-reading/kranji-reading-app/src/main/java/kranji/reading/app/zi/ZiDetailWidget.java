...
public final class ZiDetailWidget extends WorkspaceWidget<WorkspaceWidget._None, ZiDetailWidget> {
    ...
    protected List<String> constructBodyJs() {
        return List.of(
                ...
                "",
                "    // Shown only when there is more to show, and only when asked.",
                "    var more = branch.createElement('more', 'button');",
                "    css.setClass(more, kr_btn);",
                "    more.type = 'button';",
                "    root.appendChild(more);",
                "",
                "    var allHost = branch.createElement('allHost', 'div');",
                "    css.setClass(allHost, kr_grid_host, kr_kn_hidden);",
                "    root.appendChild(allHost);",
                "",
                ...
                "    // ── The grid ──────────────────────────────────────────────",
                "",
                "    // Two grids, and each gets a branch of its own: the readings grid",
                "    // names its cell branch 'readingCells', so two sharing one parent",
                "    // would dissolve each other on every render.",
                "    function gridIn(name, into) {",
                "        if (branch.getBranch(name)) branch.dissolveBranch(name);",
                "        var b = branch.createBranch(name);",
                "        b.activate(owner);",
                "        return createZiReadingsGrid({",
                "            branch: b, css: css, host: into, owner: owner,",
                ...
                "    }",
                "",
                "    // The sound that was actually selected. The rest of the character's",
                "    // readings are true and, at the moment somebody tapped one square in",
                "    // one sentence, they are noise - so they wait behind a control.",
                "    var readings = gridIn('focusGrid', host);",
                "    var everything = gridIn('allGrid', allHost);",
                "",
                "    // ── The character ─────────────────────────────────────────",
                "",
                ...
                "",
                "        readings.show(cp, narrowed ? focused : all);",
                "        everything.show(cp, all);",
                "",
                "        // The control only exists when it would reveal something. A button",
                "        // that opens an identical list is a button that teaches nobody to",
                "        // press it again.",
                "        if (narrowed) {",
                "            more.textContent = __showAll",
                "                ? 'Hide the other readings'",
                "                : 'Show all ' + all.length + ' readings';",
                "            css.setClass(more, kr_btn);",
                "            if (__showAll) css.setClass(allHost, kr_grid_host);",
                "            else           css.setClass(allHost, kr_grid_host, kr_kn_hidden);",
                "        } else {",
                "            css.setClass(more, kr_btn, kr_kn_hidden);",
                "            css.setClass(allHost, kr_grid_host, kr_kn_hidden);",
                "        }",
                "",
                "        status.textContent = !mod.polyphonic ? 'One reading.'",
                "            : narrowed",
                ...
                "            }",
                "        },",
                "        dispose: function () { __seq++; readings.destroy(); everything.destroy(); }",
                "    };");
    }
}
