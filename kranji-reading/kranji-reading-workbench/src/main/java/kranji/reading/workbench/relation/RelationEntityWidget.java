...
/**
 * One relation of a family, on a grid, wired into the cascade.
 *
 * <h2>The cascade</h2>
 *
 * <p>Each widget listens for a selection on the relation <b>directly above</b>
 * it and knows nothing else about what happened there. On hearing one it
 * reloads itself scoped to those keys, auto-selects the first row, and
 * announces that — which is what the relation below it is listening for.
 * Pick a shelf and the articles narrow to it, the first is taken, and its
 * gaps appear, all without any widget knowing the shape of the chain it sits
 * in. The chain is declared once, in the family's {@link RelationSet}.</p>
 *
 * <h2>Why keys and not objects</h2>
 *
 * <p>Only primary keys travel. A message says "article X is selected" and
 * never carries the row, so a widget cannot come to depend on the shape of
 * another's data, and a stale row cannot be broadcast as though it were
 * current. The receiving widget asks the server for what it needs, which is
 * also what makes the filtering testable in Java rather than trapped in the
 * browser.</p>
 *
 * <p>Scoping is done by the server, not by the grid: the request carries
 * {@code parent=} and the response holds only rows under it. A grid that held
 * everything and hid most of it would report the wrong count and copy the
 * wrong rows.</p>
 *
 * <h2>One widget, several families</h2>
 *
 * <p>This was the studio's gloss widget, and everything about it that named
 * the gloss family — its route, its party, its secretary, its labels — is now
 * a method a family answers. The studio answers them for gloss; the library
 * bench answers them for coverage; the JavaScript is one copy.</p>
 */
public abstract class RelationEntityWidget<SELF extends RelationEntityWidget<SELF>>
        extends WorkspaceWidget<WorkspaceWidget._None, SELF> {

    /** The relation this widget shows — one of {@code relations().relations()}. */
    protected abstract String entity();

    /** The family this relation belongs to: its chain, columns and rows. */
    protected abstract RelationSet relations();

    /** The route that serves the family's relations, e.g. {@code /gloss-relation}. */
    protected abstract String route();

    /** The party's name as declared in the workspace spec, e.g. {@code glossSelection}. */
    protected abstract String secretaryName();

    /** The party as exposed on {@code workspaceCtx}, e.g. {@code glossParty}. */
    protected abstract String partyExposedName();
    ...
    protected List<String> constructBodyJs() {
        ...
        String upstream = relations().upstreamOf(entity);
        String up = upstream == null ? "null" : "'" + upstream + "'";
        String refSource = relations().refSourceOf(entity);
        String ref = refSource == null ? "null" : "'" + refSource + "'";

        return List.of(
                "    var ENTITY = '" + entity + "';",
                "    var FAMILY = '" + relations().family() + "';",
                "    var ROUTE = '" + route() + "';",
                "    var SECRETARY = '" + secretaryName() + "';",
                "    var PARTY = '" + partyExposedName() + "';",
                "    var UPSTREAM = " + up + ";",
                ...
                "",
                "    var owner = Object.freeze({ toString: function () { return FAMILY + '/' + ENTITY; } });",
                "    var grid = null;",
                "    var seq = 0;",
                ...
                "",
                "    var __party = (workspaceCtx && workspaceCtx[PARTY])",
                "                ? workspaceCtx[PARTY] : null;",
                "    var __actorId = null;",
                "",
                "    // ── The bus ───────────────────────────────────────────────",
                "",
                ...
                "        var url = ROUTE + '?name=' + encodeURIComponent(ENTITY);",
                "        if (parentPks !== null) {",
                ...
                "",
                "    if (__party) {",
                "        __actorId = FAMILY + '-' + ENTITY + '-'",
                "                  + Math.random().toString(36).slice(2, 8);",
                "        __party.joinActor({",
                "            id: __actorId,",
                "            parentSecretary: SECRETARY,",
                "            reactors: {",
