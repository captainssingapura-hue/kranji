...
public final class SectionReaderWidget
    ...
    protected List<ModuleImports<? extends Importable>> bodyImports() {
        return List.of(
                ...
                new ModuleImports<>(List.of(
                        ...
                        new ArticleWorkbenchCss.aw_focusable(),
    ...
    protected List<String> constructBodyJs() {
        return List.of(
                ...
                "    css.setClass(list, aw_list, aw_focusable);",
                "    split.appendChild(list);",
                "",
                "    // Which of the two halves has the keyboard.",
                "    //",
                "    // Tracked here rather than read off document.activeElement,",
                "    // because in this shell it cannot be read: the workspace keeps",
                "    // focus on the body - wl-body-locked - so activeElement is BODY",
                "    // whichever half a reader last touched. A focus test would",
                "    // answer the same thing for ever, which is exactly how these two",
                "    // came to fight: the tree took every arrow and called",
                "    // preventDefault, so the grid cursor never moved.",
                "    var holder = 'tree';",
                "",
                "    // Focusable, so Tab reaches it. The grid's table is already",
                "    // tabbable, so the two of them become the pane's tab order:",
                "    // Tab goes tree, grid, out. Without this the tree could never",
                "    // hold focus - its rows are divs and TreeRenderer does not make",
                "    // them focusable - and 'which one gets the arrows' would have",
                "    // no honest answer, which is how they came to fight over them.",
                "    list.tabIndex = 0;",
                "",
                "    // A click is the other way a reader says which half they mean,",
                "    // and the unambiguous one. Recorded rather than acted on: the",
                "    // click has already done whatever it does, this only remembers",
                "    // where the arrows should go next.",
                "    list.addEventListener('click', function () { holder = 'tree'; });",
                "",
                ...
                "    boardHost.addEventListener('click', function () { holder = 'grid'; });",
                "",
                ...
                "    var __renderer = null;",
                "",
                "    // Whether the keys are this PANE's at all. Which half of it",
                "    // they then belong to is a separate question, answered below -",
                "    // this one only keeps the pane next door out of it, whose tab",
                "    // content contains its own root and not this one.",
                "    function ours() {",
                ...
                "",
                "",
                "    function toTree() { holder = 'tree'; try { list.focus(); } catch (e) {} }",
                "    function toGrid() { holder = 'grid'; sheet.focus(); }",
                "",
                "    var __keys = function (ev) {",
                "        if (!ours()) return;",
                "",
                "        // Tab is how a reader says which half they mean. Two things",
                "        // want the arrows and only a person can settle it, so it is",
                "        // a gesture rather than a guess.",
                "        if (ev.key === 'Tab' && !ev.ctrlKey && !ev.altKey && !ev.metaKey) {",
                "            if (holder === 'tree') toGrid(); else toTree();",
                "            ev.preventDefault();",
                "            return;",
                "        }",
                "",
                "        // The grid's keys are the grid's. It handles them itself,",
                "        // which it can only do if nothing above swallows them first.",
                "        if (holder !== 'tree') return;",
