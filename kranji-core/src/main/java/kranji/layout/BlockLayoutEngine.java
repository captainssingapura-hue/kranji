...
public final class BlockLayoutEngine {
    ...
    /** Append a child slot index to a dotted path. */
    ...
    static List<Block> applyGravity(List<Block> blocks) {
        ...
        return blocks.stream()
                .map(b -> new Block(b.glyph(), b.x() + fdx, b.y() + fdy, b.w(), b.h(), b.depth(), b.role(), b.hint(), b.path()))
                .toList();
    }
    ...
    private static List<Block> layoutBlock(
            ...
            int depth, BlockRole role, String fallbackGlyph, String path
    ) {
        ...
        if (node instanceof SingularBlock sb) {
            if (role != null) {
                ...
                return List.of(new Block(glyph, x, y, w, h, depth, role, hint, path));
            }
            // No role (top-level singular entry) — use glyph directly.
            String glyph = (fallbackGlyph != null) ? fallbackGlyph : sb.glyph();
            return List.of(new Block(glyph, x, y, w, h, depth, new SingularRole(), null, path));
        }
        if (node instanceof ComposedBlock comp) {
            return layoutComposition(comp.composition(), x, y, w, h, depth, fallbackGlyph, path);
        }
        return List.of();
    }
    ...
    private static List<Block> layoutComposition(
            ...
            double x, double y, double w, double h,
            int depth, String fallbackGlyph, String path
    ) {
        List<Block> result = new ArrayList<>();
        // Slot index 0 / 1 (/ 2) for two-/three-way splits — matches
        // CompositionLayout.components() ordering so paths resolve back
        // via the same component list.
        String p0 = childPath(path, 0);
        String p1 = childPath(path, 1);
        String p2 = childPath(path, 2);
        ...
        switch (comp) {
            case LeftRight(var left, var right) -> {
                ...
                result.addAll(layoutBlock(left, x, y, splitX, h, depth + 1, LeftRight.LEFT, null, p0));
                result.addAll(layoutBlock(right, x + splitX, y, w - splitX, h, depth + 1, LeftRight.RIGHT, null, p1));
            }
            ...
            case TopBottom(var top, var bottom) -> {
                ...
                result.addAll(layoutBlock(top, x, y, w, splitY, depth + 1, TopBottom.TOP, null, p0));
                result.addAll(layoutBlock(bottom, x, y + splitY, w, h - splitY, depth + 1, TopBottom.BOTTOM, null, p1));
            }
            ...
            case LeftMiddleRight(var l, var m, var r) -> {
                ...
                result.addAll(layoutBlock(l, x, y, w1, h, depth + 1, LeftMiddleRight.LEFT, null, p0));
                result.addAll(layoutBlock(m, x + w1, y, w2, h, depth + 1, LeftMiddleRight.MIDDLE, null, p1));
                result.addAll(layoutBlock(r, x + w1 + w2, y, w3, h, depth + 1, LeftMiddleRight.RIGHT, null, p2));
            }
            ...
            case TopMiddleBottom(var t, var m, var b) -> {
                ...
                result.addAll(layoutBlock(t, x, y, w, h1, depth + 1, TopMiddleBottom.TOP, null, p0));
                result.addAll(layoutBlock(m, x, y + h1, w, h2, depth + 1, TopMiddleBottom.MIDDLE, null, p1));
                result.addAll(layoutBlock(b, x, y + h1 + h2, w, h3, depth + 1, TopMiddleBottom.BOTTOM, null, p2));
            }

            case FullEnclosure(var outer, var inner) -> {
                String outerGlyph = outer.glyph();
                result.add(new Block(outerGlyph, x, y, w, h, depth + 1, FullEnclosure.OUTER, null, p0));
                double mx = w * ENCL_MARGIN, my = h * ENCL_MARGIN;
                result.addAll(layoutBlock(inner, x + mx, y + my, w - 2 * mx, h - 2 * my,
                        depth + 1, FullEnclosure.INNER, null, p1));
            }

            case SemiEnclosureBottomLeft(var wrapper, var content) -> {
                String wGlyph = wrapper.glyph();
                result.add(new Block(wGlyph, x, y, w, h, depth + 1, SemiEnclosureBottomLeft.WRAPPER, null, p0));
