// AUTO-GENERATED by Depth1GenerateMain - do not edit by hand.
package kranji.common.depth1.l;

import kranji.classification.EtymologicalCategory.*;
import kranji.component.basic.BasicComponents;
import kranji.pinyin.Body;
import kranji.pinyin.Final;
import kranji.pinyin.Head;
import kranji.pinyin.Initial;
import kranji.pinyin.PinyinSyllable;
import kranji.pinyin.Tail;
import kranji.pinyin.Tone;
import kranji.singular.actions.ActionsAndStates;
import kranji.singular.nature.NatureElements;
import kranji.zi.ComposedPart;
import kranji.zi.ComposedZi;
import kranji.zi.CompositionLayout.*;
import kranji.zi.SingularPart;
import kranji.zi.ZiChar;
import java.util.List;

public final class A1 {

    private A1() {}

    public static final ComposedZi LA_GARBAGE = new ComposedZi(
            new ZiChar.Unicode("\u5783"),
            new PinyinSyllable(Initial.L, new Final(Head.OPEN, Body.A, Tail.NONE), Tone.FIRST),
            8, 32, "",
            new LeftRight(NatureElements.TU, ActionsAndStates.LI_STAND),
            new PhonoSemantic(NatureElements.TU, ActionsAndStates.LI_STAND));

    public static final ComposedZi LA_PULL = new ComposedZi(
            new ZiChar.Unicode("\u62C9"),
            new PinyinSyllable(Initial.L, new Final(Head.OPEN, Body.A, Tail.NONE), Tone.FIRST),
            8, 64, "",
            new LeftRight(BasicComponents.TI_SHOU_PANG, ActionsAndStates.LI_STAND),
            new PhonoSemantic(BasicComponents.TI_SHOU_PANG, ActionsAndStates.LI_STAND));

    public static final List<ComposedZi> ALL = List.of(
            LA_GARBAGE,
            LA_PULL);
}
